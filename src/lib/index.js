...
class Bell {
  ...
  constructor(text, type, options) {
    this.text = text;
    this.type = type;
    const {animate=true,isColored=true,transitionDuration=300,position="bottom-right",typeAnimation="fade-in",timeScreen=3000,expand=false,theme="default",customClass,customStyles,customIcon,customHTML,timeline,removeOn} = options
    this.transitionDuration = transitionDuration ?? 300
    this.position = position.split("-");
    this.classPosition = this.position.join(".")
    this.timeScreen = this.type == "promise" ? 100000 : timeScreen;
    this.expand = expand;
    ...
    if(customIcon) $icons[type] = customIcon
    ...
    if(customStyles){
      ...
      keys.forEach(key => {
        const element = key == "container" ? this.$bellContainer : key == "title" ? this.$bellTitle : key == "description" ? this.$bellDescription : this.$bellIcon
        if(element)Object.entries(customStyles[key]).forEach(val => element.style[val[0]] = val[1])})
    }
  ...
  #changeState(state,$bell,states){
      this.$bellIcon.innerHTML = $icons[state]
  ...
  setPositions($bellNums, isRemove) {
    ...
    let firstBell = $bellNums[length -1]
    if (firstBell) {
      firstBell.style.scale = "1"
    }
    if (
      $bellPrev?.className.includes("top")
    ) {
    ...
    if (length > 2) {
      ...
      if ($bellPrevPrev) $bellPrevPrev.style.scale = ".88";
      if ($bellPrevPrev?.className.includes("top")
      )
        $bellPrevPrev.style.top = "60px";
      if (
        $bellPrevPrev?.className.includes("bottom")
      )
        $bellPrevPrev.style.bottom = "60px";
    }
    let top = 30;
    const $bellNumsArray = [...$bellNums]
    if ($bellNums && $bellNums[length - 1]) bellState.maxHeight = $bellNums[length - 1]?.getAttribute("bell-height");
  ...
  launch(isPromise) {
    ...
    setTimeout(() => {
      if(this.removeEvent == "click"){
        this.$bellContainer.addEventListener("click", this.removeOfScreen.bind(this))
      }
...
`
      ...
      $closeB.addEventListener("click", this.removeOfScreen.bind(this))
      }
